...
void vprintf_color(const char* format, va_list args)
{
    ...
    printf(") ");
#endif
    size_t size = (vsnprintf(NULL, 0, format, args) + 1) * sizeof(char);
...
void pos(const uint8_t x, const uint8_t y, const char* data)
{
    printf(POS("%s", "%d", "%d"), x, y, data);
}

void back(const char* data, const uint8_t number)
{
    printf(BACK("%s", "%d"), number, data);
}

void forward(const char* data, const uint8_t number)
{
    printf(FORWARD("%s", "%d"), number, data);
}

void down(const char* data, const uint8_t number)
{
    printf(DOWN("%s", "%d"), number, data);
}

void up(const char* data, const uint8_t number)
{
...
static void foreground_color_custom_(const uint8_t red, const uint8_t green, const uint8_t blue)
{
...
static void background_color_custom_(const uint8_t red, const uint8_t green, const uint8_t blue)
{
...
// no comptible para win7
static void background_color_custom_RGB(RGB_C color) { return; }
static void foreground_color_custom_RGB(RGB_C color) { return; }
static void background_color_custom_(const uint8_t red, const uint8_t green, const uint8_t blue) { return; }
static void foreground_color_custom_(const uint8_t red, const uint8_t green, const uint8_t blue) { return; }
#endif
...
static void back_fore_color_custom_(uint8_t redB, uint8_t greenB,
...
void print_bin(const void* data, size_t size)
{
    ...
    for (size_t byte = 0; byte < size; byte++)
        for (size_t i = 8; i > 0; i--) {
            uint8_t mask = 1U << (i - 1);
            uint8_t bit = bytePtr[byte] & mask ? '1' : '0';
            putchar(bit);
        }
}
...
void print_table_hex(char* string_init, char* string_text_for_printing, size_t size_string_text_for_printing)
{
    ...
    if ((string_text_for_printing == NULL) || (size_string_text_for_printing == 0))
        return; // error
    if (string_init == NULL)
        string_init = my_string_default; // no se ingreso un valor de incio que usar para imprimir cada fila
    ...
    for (uint32_t i = 0; i < level_space; i++) {
        putchar(' ');
    }

#ifndef __DISABLE_COLORS_FORE_BACK_GROUND__
    uint32_t random_color = jenkins_hash(string_text_for_printing[0], level_space, 0, 1, 2, 3, 4);
    ...
    // 0x77: 0b1110111
    // 0x09: 0b0001001
    printf_color("|\n%s" FOREGROUND_COLOR_CUSTOM("%d") "%s #{FG:reset}", string_init, ((((uint8_t)random_color >> 2)) & 0x77) | 0x09, buffer_Position_memory);

#else
    for (uint16_t r = 0x23; r < 0x33; r++)
        printf_color("|%02x#{FG:reset}", r >> 2);
    ...
    for (uint32_t i = 0; i < size_string_text_for_printing; i++) {
        ...
        uint32_t Avalue1, Avalue2, Avalue3, seed, values[] = { (unsigned int)size_string_text_for_printing, size, level_space, 0xa0, 0xe1, string_text_for_printing[i] };
        // generar 3 valores apartir de una semilla de entrada y 6 valores(pueden ser constantes o variables)
        seed = (uint32_t)(string_text_for_printing[i]);
        ...
        printf_color("|" FOREGROUND_COLOR_CUSTOM("%d") "%.2X#{BG:reset}",
            // 0x77: 0b1110111
            // 0x09: 0b0001001
            ((((uint8_t)string_text_for_printing[i] >> 2)) & 0x77) | 0x09,
        ...
        if ((i + 1) % ((256 * 4) / 8 / 8) == 0) {
            ...
            // 0x7F: 0b1111111
            // 0x09: 0b0001001
            printf_color("|\n%s" FOREGROUND_COLOR_CUSTOM("%d") "%s #{FG:reset}", string_init, ((((uint8_t)random_color >> 2)) & 0x7F) | 0x09, buffer_Position_memory, i + 1);
#else
